...
async function build(rootDir: string) {
  try {
    ...
    const distFamiconsDir = join(distDir, 'famicons');
    const distSvgDir = join(distDir, 'svg');

    await Promise.all([fs.emptyDir(iconDir), fs.emptyDir(distDir)]);
    await fs.emptyDir(distSvgDir), await fs.emptyDir(distFamiconsDir);

    const pkgData = await fs.readJson(pkgJsonPath);
    const version = pkgData.version as string;

    const srcSvgData = await getSvgs(srcSvgDir, distSvgDir, distFamiconsDir);
          ...
          fn: (item, params) => {
            ...
            if (item.isElem()) {
              item.eachAttr((attr) => {
                if (attr.name === 'fill') {
                  if (attr.value === 'none') {
                    item.class.add('famicon-fill-none');
                  }
                  ...
                  item.class.add('famicon-stroke-width');
                }
              });
            }
          },
...
async function optimizeSvg(
  ...
  const optimizedCode = optimizedSvg.data.replace(
    /<svg (.*?)>/,
    `<svg xmlns="http://www.w3.org/2000/svg" class="famicon" viewBox="0 0 512 512"><title>${svgData.title}</title>`,
  );
...
async function createSvgSymbols(version: string, distDir: string, srcSvgData: SvgData[]) {
  ...
  const symbolsSvgFilePath = join(distDir, 'famicons.symbols.svg');

  const lines = [
    `<svg data-famicons="${version}" style="display:none">`,
    `<style>`,
    `.famicon {`,
    `  fill: currentColor;`,
    `  stroke: currentColor;`,
...
async function getSvgs(srcSvgDir: string, distSvgDir: string, distFamiconsDir: string): Promise<SvgData[]> {
  const optimizedSvgDir = join(distFamiconsDir, 'svg');
  ...
  const svgData = await Promise.all(
    svgFiles.map(async (fileName) => {
      ...
      // optimizedFilePath: /dist/famicons/svg/airplane-outline.svg
...
async function createEsmIcons(version: string, iconDir: string, srcSvgData: SvgData[]) {
  ...
  const o = [`/* Famicons v${version}, ES Modules */`, ``];
...
async function createCjsIcons(version: string, iconDir: string, srcSvgData: SvgData[]) {
  ...
  const o = [`/* Famicons v${version}, CommonJS */`, ``];
...
async function createDtsIcons(version: string, iconDir: string, srcSvgData: SvgData[]) {
  ...
  const o = [`/* Famicons v${version}, Types */`, ``];
...
async function createDataJson(version: string, srcDir: string, distDir: string, srcSvgData: SvgData[]) {
  ...
  const distDataJsonPath = join(distDir, 'famicons.json');
...
interface SvgData {
  ...
  /**
   * /dist/famicons/svg/airplane-outline.svg
   */
