...
async function build(rootDir: string) {
  try {
    ...
    await fs.emptyDir(distFamiconsDir);

    const pkgData = await fs.readJson(pkgJsonPath);
    const version = pkgData.version as string;

    const srcSvgData = await getSvgs(srcSvgDir, rootDir, distFamiconsDir)
...
async function getSvgs(srcSvgDir: string, rootDir: string, distFamiconsDir: string): Promise<SvgData[]> {
